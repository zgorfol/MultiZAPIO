...
TARGET_F = 87975  # In Hz
CLOCK_MCU = 80000000  # 80Mhz CPU clock / BIOZAP_SAMPLE_SIZE
TOLERANCE = 0.001
...
def close_divisor(psc, tolerance):
    ...
    if error < tolerance and arr < 65536.0 and arr > 12:
        h = hertz(CLOCK_MCU, psc, int(arr))
        return psc, int(arr), h, error, lnko(psc, int(arr))
    else:
        return None
...
df = pd.DataFrame(columns=['PSC', 'ARR', 'F', 'ERROR', 'LNKO'], dtype=np.double)
...
for index in range(len(poss_prescalers)):
    ...
    if value is not None:
        close_prescalers.append((value[0], value[1], value[2], value[3], value[4]))
df = df.append(pd.DataFrame(np.array(close_prescalers).reshape(len(close_prescalers), 5), columns=df.columns))
...
df = df.sort_values(by=['LNKO', 'ERROR'])
...
df['ERROR'] = df['ERROR'].map('{:.15f}'.format)
df['LNKO'] = df['LNKO'].map('{:.0f}'.format)
